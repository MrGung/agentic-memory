...
(defn- ensure-db! []
  (let [db (open-db)]
    (try
      ...
      (sqlite/execute! db ["CREATE INDEX IF NOT EXISTS idx_events_session ON events (session)"])
      (sqlite/execute! db ["CREATE INDEX IF NOT EXISTS idx_events_type ON events (type)"])
      (sqlite/execute! db ["CREATE INDEX IF NOT EXISTS idx_events_valid_time ON events (valid_time)"])
      (try (sqlite/execute! db ["ALTER TABLE events ADD COLUMN transaction_time TEXT"]) (catch Exception _ nil))
      (try (sqlite/execute! db ["ALTER TABLE events ADD COLUMN valid_time TEXT"]) (catch Exception _ nil))
      (sqlite/execute! db ["UPDATE events SET transaction_time = COALESCE(transaction_time, timestamp)"])
      (sqlite/execute! db ["UPDATE events SET valid_time = COALESCE(valid_time, timestamp, transaction_time)"])
      (finally
        (sqlite/close db)))))
...
(defn- memory-search [{:keys [query cross_session event_type]}]
  (let [db         (open-db)
        conditions (cond-> [["LOWER(data) LIKE ? ESCAPE '\\'" (like-pattern query)]]
